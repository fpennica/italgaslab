...
import it.cnr.igag.italgas.domain.PersistentToken;
import it.cnr.igag.italgas.domain.User;
import it.cnr.igag.italgas.repository.PersistentTokenRepository;
...
import it.cnr.igag.italgas.service.MailService;
import it.cnr.igag.italgas.service.OperatoreService;
import it.cnr.igag.italgas.service.UserService;
import it.cnr.igag.italgas.web.rest.dto.KeyAndPasswordDTO;
import it.cnr.igag.italgas.web.rest.dto.ManagedUserDTO;
import it.cnr.igag.italgas.web.rest.dto.UserDTO;
...
//                .orElse(new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR));
    	Operatore operatore = operatoreService.findByLogin();
    	User user = userService.getUserWithAuthorities();
    	UserDTO userDTO = new UserDTO(user);
    	userDTO.setLaboratorio(operatore.getLaboratorio());
        return Optional.ofNullable(user)
            .map(u -> new ResponseEntity<>(userDTO, HttpStatus.OK))
            .orElse(new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR));
    }
    ...
    public ResponseEntity<?> requestPasswordReset(@RequestBody String mail, HttpServletRequest request) {
        return userService.requestPasswordReset(mail)
            .map(user -> {
                ...
                //probabilmente per un problema con il reverse proxy baseUrl risulta sempre localhost:8080
                //http://stackoverflow.com/questions/19598690/how-to-get-host-name-with-port-from-a-http-or-https-request
                //andrebbero usati header settati da apache del tipo:
                //log.info(request.getHeader("x-forwarded-proto"));
                mailService.sendPasswordResetMail(user, "http://dune.igag.cnr.it/italgaslab");
    ...
    @RequestMapping(value = "/account/operatore",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<Operatore> getOperatoreByLogin() {
        log.debug("REST request to get Operatore by login name");
        Operatore operatore = operatoreService.findByLogin();
